...
class Assignment3:
    ...
    def areabetween(self, f1: callable, f2: callable) -> np.float32:
        ...
        abs = self.abs_intersect_function(f1, f2)
        find = self.find_intersection_f(f1, f2)

        area_between_functions = 0
        num_of_points = len(find)
        for i in range(num_of_points-1):
            area_between_functions += self.integrate(abs, find[i], find[i+1], 100)
    ...
    def find_intersection_f(self, f1, f2): #find roots
        as2= assignment2.Assignment2()
        find = as2.intersections(f1, f2, 1, 100) #call the function from assignment2 that find roots
        return find
